...
foreach($_FILES['file']['tmp_name'] as $key =>
 $tmp_name ){
        ...
        $file_name = $_FILES['file']['name'][$key]."<br/>";
 
        $file_size =$_FILES['file']['size'][$key]."<br/>";
 
        $file_tmp_name =$_FILES['file']['tmp_name'][$key]."<br/>";
 
       $file_type=$_FILES['file']['type'][$key]."<br/>";
...
$BigImageMaxSize 		= $file_size; //Image Maximum height or width
$ThumbPrefix			= "thumb_"; //Normal thumb Prefix
$DestinationDirectory	= '../images/gallery/'; //Upload Directory ends with / (slash)
$Quality 				= 100;
...
if (is_array($ImageName)) 
{
	...
	{
		$processImage			= true;	
		$RandomNumber			= rand();  // We need same random name for both files.
...
//					echo '</table>';
					...
					$sql = "INSERT INTO gallery (path, ThumbName, ImgPath, title, img_title, alt, s_no) VALUES ('$DestRandImageName', '$thumb_DestRandImageName', '$ImageName' , '$title', '$img_title', '$alt', '$s_no');";
